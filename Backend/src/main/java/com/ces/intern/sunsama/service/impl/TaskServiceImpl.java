...
public class TaskServiceImpl implements TaskService {
    ...
    public TaskDTO updateTask(long id, TaskRequest request) {
        ...
        List<HashtagEntity> newHashtags = hashtagRepository.findAllById(request.getHashtagsId());
        task.setTitle(request.getTitle());
        task.setDescribe(request.getDescribe());
        task.setDate(request.getDate());
        task.setDueDate(request.getDueDate());
        task.setHashtags(newHashtags);
...
//        taskRepository.deleteAllSubtasksOfTask(taskEntity.getId());
        List<TaskEntity> subtasks = taskRepository.getSubtaskOfTask(taskEntity.getId());
    ...
    @Override
    public List<TaskDTO> getTasksByDateWithHashtag(String dateStr, Long hashtagId) {
        List<TaskEntity> taskEntities = taskRepository.getTasksByDate(dateStr);
        if(hashtagId != null){
            HashtagEntity hashtag = hashtagRepository.findById(hashtagId.longValue()).orElseThrow(() -> new NotFoundException("Hashtag not found"));
            taskEntities = taskEntities.stream().filter(task -> task.getHashtags().contains(hashtag)).toList();
        }
    ...
    public void setTaskComplete(long taskId) {
        ...
        if(task.getParentId() == 0 && currentStatus == false){
            ...
            if(task.getParentId() != 0 && currentStatus==true){
                TaskEntity parentTask = taskRepository.findById(task.getParentId()).get();
                parentTask.setStatus(false);
            }
    ...
    @Override
    @Transactional
    public void changeDateOfTask(long taskId, String date) {
        TaskEntity task = taskRepository.findById(taskId).orElseThrow(() -> new NotFoundException("Task not found"));
        String oldDateStr = task.getDate().toString();
        String time = oldDateStr.split(" ")[1];
        String newDateStr = date + " "+ time;
        Date newDate = DateValidator.toDate(newDateStr);
